...
const TestTypesPage = ({ route, onEdit, refreshTrigger, setEditingItem, setEditingEndpoint, setEditingCrudEndpoint, setModalType }) => {
  const [isHistModalOpen, setIsHistModalOpen] = useState(false);
  const [histData, setHistData] = useState([]);
  const [loading, setLoading] = useState(false);
  const [selectedTtCode, setSelectedTtCode] = useState(null);

  const fetchHistory = async (ttCode = null) => {
    setLoading(true);
    try {
      const endpoint = ttCode ? `/hist-test-types?tt_code=${ttCode}` : `/hist-test-types`;
  ...
  return (
    <>
      ...
      <Modal 
        ...
        title={`Riwayat Perubahan Jenis Uji ${selectedTtCode ? `(Kode: ${selectedTtCode})` : "Keseluruhan"}`}
        wide
      >
