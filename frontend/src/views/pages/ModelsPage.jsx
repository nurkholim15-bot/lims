...
const ModelsPage = ({ route, refreshTrigger, checkPasswordRequirement }) => {
  // History State
  ...
  const fetchHistory = async (modelCode = null) => {
    setHistLoading(true);
    try {
      const endpoint = modelCode ? `/hist-models?model_code=${modelCode}` : `/hist-models`;
  ...
  const handleRowClick = (item) => {
    setSelectedModelCode(item.code);
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: "1rem" }}>
      <MasterDataPage 
        title={route?.title || "Model Alat"} 
        endpoint={route?.endpoint || "/models"} 
        crudEndpoint={route?.crudEndpoint || "/management/models"} 
      ...
      <Modal 
        ...
        title={`Riwayat Perubahan Model Alat ${selectedModelCode ? `(Kode: ${selectedModelCode})` : "Keseluruhan"}`}
        wide
      >
            ...
            <table style={{ borderCollapse: "separate", borderSpacing: 0, width: "100%", fontSize: "0.875rem" }}>
              <thead style={{ position: "sticky", top: 0, zIndex: 10, background: "white" }}>
                <tr>
                  ...
                  <th style={{ background: "white", borderBottom: "2px solid #f1f5f9" }}>Brand</th>
