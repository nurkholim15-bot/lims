...
const PartnerTypesPage = ({ route, refreshTrigger }) => {
  // History State
  const [isHistModalOpen, setIsHistModalOpen] = useState(false);
  const [histData, setHistData] = useState([]);
  const [histLoading, setHistLoading] = useState(false);
  const [selectedPtCode, setSelectedPtCode] = useState(null);

  const fetchHistory = async (ptCode = null) => {
    setHistLoading(true);
    try {
      const endpoint = ptCode ? `/hist-partner-types?pt_code=${ptCode}` : `/hist-partner-types`;
  ...
  const columns = [
    ...
    { key: "name", header: "Nama Jenis Partner" },
  ];
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: "1rem" }}>
      <MasterDataPage 
        title={route?.title || "Jenis Partner"} 
        endpoint={route?.endpoint || "/partner-types"} 
        crudEndpoint={route?.crudEndpoint || "/management/partner-types"} 
      ...
      <Modal 
        ...
        title={`Riwayat Perubahan Jenis Partner ${selectedPtCode ? `(Kode: ${selectedPtCode})` : "Keseluruhan"}`}
        wide
      >
