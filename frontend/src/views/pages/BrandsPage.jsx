...
const BrandsPage = ({ route, refreshTrigger, checkPasswordRequirement }) => {
  // History State
  ...
  const fetchHistory = async (brandCode = null) => {
    setHistLoading(true);
    try {
      const endpoint = brandCode ? `/hist-brands?brand_code=${brandCode}` : `/hist-brands`;
  ...
  const handleRowClick = (item) => {
    setSelectedBrandCode(item.code);
  ...
  const columns = [
    ...
    { key: "name", header: "Nama Merk" },
    { key: "material_category_code", header: "Kategori", render: (item) => item.material_category?.name || item.material_category_code || "-" },
    { key: "origin_code", header: "Negara", render: (item) => item.origin?.name || item.origin_code || "-" },
  ];
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: "1rem" }}>
      <MasterDataPage 
        title={route?.title || "Merk Peralatan"} 
        endpoint={route?.endpoint || "/brands"} 
        crudEndpoint={route?.crudEndpoint || "/management/brands"} 
      ...
      <Modal 
        ...
        title={`Riwayat Perubahan Merk Peralatan ${selectedBrandCode ? `(Kode: ${selectedBrandCode})` : "Keseluruhan"}`}
        wide
      >
            ...
            <table style={{ borderCollapse: "separate", borderSpacing: 0, width: "100%", fontSize: "0.875rem" }}>
              <thead style={{ position: "sticky", top: 0, zIndex: 10, background: "white" }}>
                <tr>
                  ...
                  <th style={{ background: "white", borderBottom: "2px solid #f1f5f9" }}>Kategori</th>
                  <th style={{ background: "white", borderBottom: "2px solid #f1f5f9" }}>Origin</th>
              ...
              <tbody>
                {histLoading ? (
                  ...
                  histData.map((item) => (
                    <tr key={item.id}>
                      <td>{item.id}</td>
                      <td>{item.brand_code_ref}</td>
                      <td>{item.code}</td>
                      <td>{item.name}</td>
                      <td>{item.material_category_code || "-"}</td>
