...
const PartnerCategoriesPage = ({ route, refreshTrigger }) => {
  // History State
  ...
  const fetchHistory = async (pcCode = null) => {
    setHistLoading(true);
    try {
      const endpoint = pcCode ? `/hist-partner-categories?pc_code=${pcCode}` : `/hist-partner-categories`;
  ...
  const columns = [
    ...
    { key: "name", header: "Nama Kategori Partner" },
  ];
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: "1rem" }}>
      <MasterDataPage 
        title={route?.title || "Kategori Partner"} 
        endpoint={route?.endpoint || "/partner-categories"} 
        crudEndpoint={route?.crudEndpoint || "/management/partner-categories"} 
      ...
      <Modal 
        ...
        title={`Riwayat Perubahan Kategori Partner ${selectedPcCode ? `(Kode: ${selectedPcCode})` : "Keseluruhan"}`}
        wide
      >
...
export default PartnerCategoriesPage;
