...
const ProvincesPage = ({ route, refreshTrigger }) => {
  // History State
  ...
  const fetchHistory = async (provinceCode = null) => {
    setHistLoading(true);
    try {
      const endpoint = provinceCode ? `/hist-provinces?province_code=${provinceCode}` : `/hist-provinces`;
  ...
  const handleRowClick = (item) => {
    setSelectedProvinceCode(item.province_code);
    setIsHistModalOpen(true);
    fetchHistory(item.province_code);
  };
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: "1rem" }}>
      <MasterDataPage 
        title={route?.title || "Daftar Provinsi"} 
        endpoint={route?.endpoint || "/provinces"} 
        crudEndpoint={route?.crudEndpoint || "/management/provinces"} 
      ...
      <Modal 
        ...
        title={`Riwayat Perubahan Provinsi ${selectedProvinceCode ? `(Kode: ${selectedProvinceCode})` : "Keseluruhan"}`}
        wide
      >
