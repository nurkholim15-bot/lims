...
const MaterialCategoriesPage = ({ route, refreshTrigger }) => {
  // History State
  ...
  const fetchHistory = async (mcCode = null) => {
    setHistLoading(true);
    try {
      const endpoint = mcCode ? `/hist-material-categories?mc_code=${mcCode}` : `/hist-material-categories`;
  ...
  const columns = [
    ...
    { key: "name", header: "Nama Kategori Material" },
  ];
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: "1rem" }}>
      <MasterDataPage 
        title={route?.title || "Kategori Material"} 
        endpoint={route?.endpoint || "/material-categories"} 
        crudEndpoint={route?.crudEndpoint || "/management/material-categories"} 
      ...
      <Modal 
        ...
        title={`Riwayat Perubahan Kategori Material ${selectedMcCode ? `(Kode: ${selectedMcCode})` : "Keseluruhan"}`}
        wide
      >
