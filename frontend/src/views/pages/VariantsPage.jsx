...
const VariantsPage = ({ route, refreshTrigger, checkPasswordRequirement }) => {
  // History State
  ...
  const fetchHistory = async (variantCode = null) => {
    setHistLoading(true);
    try {
      const endpoint = variantCode ? `/hist-variants?variant_code=${variantCode}` : `/hist-variants`;
  ...
  const handleRowClick = (item) => {
    setSelectedVariantCode(item.code);
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: "1rem" }}>
      <MasterDataPage 
        title={route?.title || "Varian Alat"} 
        endpoint={route?.endpoint || "/variants"} 
        crudEndpoint={route?.crudEndpoint || "/management/variants"} 
      ...
      <Modal 
        ...
        title={`Riwayat Perubahan Varian Alat ${selectedVariantCode ? `(Kode: ${selectedVariantCode})` : "Keseluruhan"}`}
        wide
      >
            ...
            <table style={{ borderCollapse: "separate", borderSpacing: 0, width: "100%", fontSize: "0.875rem" }}>
              <thead style={{ position: "sticky", top: 0, zIndex: 10, background: "white" }}>
                <tr>
                  ...
                  <th style={{ background: "white", borderBottom: "2px solid #f1f5f9" }}>Model</th>
