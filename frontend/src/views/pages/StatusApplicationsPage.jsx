...
const StatusApplicationsPage = ({ route, onEdit, refreshTrigger, setEditingItem, setEditingEndpoint, setEditingCrudEndpoint, setModalType }) => {
  ...
  const fetchHistory = async (statusCode = null) => {
    setLoading(true);
    try {
      const endpoint = statusCode ? `/hist-status-applications?status_code=${statusCode}` : `/hist-status-applications`;
  ...
  return (
    <>
      ...
      <Modal 
        ...
        title={`Riwayat Perubahan Status Pengajuan ${selectedStatusCode ? `(Kode: ${selectedStatusCode})` : "Keseluruhan"}`}
        wide
      >
            ...
            <table style={{ borderCollapse: "separate", borderSpacing: 0, width: "100%", fontSize: "0.875rem" }}>
              <thead style={{ position: "sticky", top: 0, zIndex: 10, background: "white" }}>
                <tr>
                  ...
                  <th style={{ background: "white", borderBottom: "2px solid #f1f5f9" }}>Kode Status</th>
                  <th style={{ background: "white", borderBottom: "2px solid #f1f5f9" }}>Keterangan</th>
