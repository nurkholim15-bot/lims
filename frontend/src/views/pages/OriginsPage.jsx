...
const OriginsPage = ({ route, refreshTrigger, checkPasswordRequirement }) => {
  // History State
  ...
  const fetchHistory = async (originCode = null) => {
    setHistLoading(true);
    try {
      const endpoint = originCode ? `/hist-origins?origin_code=${originCode}` : `/hist-origins`;
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: "1rem" }}>
      <MasterDataPage 
        title={route?.title || "Asal Alat"} 
        endpoint={route?.endpoint || "/origins"} 
        crudEndpoint={route?.crudEndpoint || "/management/origins"} 
      ...
      <Modal 
        ...
        title={`Riwayat Perubahan Asal Alat ${selectedOriginCode ? `(Kode: ${selectedOriginCode})` : "Keseluruhan"}`}
        wide
      >
