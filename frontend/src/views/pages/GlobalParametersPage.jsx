...
const GlobalParametersPage = ({ route, onEdit, refreshTrigger, setEditingItem, setEditingEndpoint, setEditingCrudEndpoint, setModalType }) => {
  const [isHistModalOpen, setIsHistModalOpen] = useState(false);
  const [histData, setHistData] = useState([]);
  const [loading, setLoading] = useState(false);
  const [selectedGpId, setSelectedGpId] = useState(null);

  const fetchHistory = async (gpId = null) => {
    setLoading(true);
    try {
      const endpoint = gpId ? `/hist-global-parameters?gp_id=${gpId}` : `/hist-global-parameters`;
  ...
  return (
    <>
      ...
      <Modal 
        ...
        title={`Riwayat Perubahan ${selectedGpId ? `(ID: ${selectedGpId})` : "Keseluruhan"}`}
        wide
      >
            ...
            <table style={{ borderCollapse: "separate", borderSpacing: 0, width: "100%", fontSize: "0.875rem" }}>
              <thead style={{ position: "sticky", top: 0, zIndex: 10, background: "white" }}>
                <tr>
                  ...
                  <th style={{ background: "white", borderBottom: "2px solid #f1f5f9" }}>Ref ID</th>
                  <th style={{ background: "white", borderBottom: "2px solid #f1f5f9" }}>Key</th>
                  <th style={{ background: "white", borderBottom: "2px solid #f1f5f9" }}>Value</th>
