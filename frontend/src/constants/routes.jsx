...
export const getMasterRoutes = (onNavigate) => ({
  ...
  "/testing-packages": {
    ...
    columns: [
      ...
      { key: "aspect_count", header: "Aspek", render: (item) => item.active_aspects?.length || 0 },
      { key: "sub_aspect_count", header: "Sub-Aspek", render: (item) => item.active_sub_aspects?.length || 0 },
      { key: "is_active", header: "Status", render: (item) => item.is_active ? "Aktif" : "Non-Aktif" },
    ],
  },
  "/package-active-aspects": {
    title: "Aspek Aktif Paket",
    endpoint: "/package-active-aspects",
    crudEndpoint: "/management/package-active-aspects",
    searchField: "search",
    searchPlaceholder: "Cari Paket atau Aspek...",
    columns: [
      { key: "package_id", header: "ID Paket" },
      { key: "package_name", header: "Nama Paket", render: (item) => item.package ? `${item.package.name} (${item.package.package_code})` : `Paket #${item.package_id}` },
      { key: "aspect_code", header: "Kode Aspek" },
      { key: "aspect_name", header: "Nama Aspek", render: (item) => item.aspect ? item.aspect.name : "-" },
      { key: "created_at", header: "Waktu Ditambahkan", render: (item) => item.created_at ? new Date(item.created_at).toLocaleString("id-ID") : "-" },
    ],
  },
  "/package-active-sub-aspects": {
    title: "Sub-Aspek Aktif Paket",
    endpoint: "/package-active-sub-aspects",
    crudEndpoint: "/management/package-active-sub-aspects",
    searchField: "search",
    searchPlaceholder: "Cari Paket atau Sub-Aspek...",
    columns: [
      { key: "package_id", header: "ID Paket" },
      { key: "package_name", header: "Nama Paket", render: (item) => item.package ? `${item.package.name} (${item.package.package_code})` : `Paket #${item.package_id}` },
      { key: "sub_aspect_code", header: "Kode Sub-Aspek" },
      { key: "sub_aspect_name", header: "Nama Sub-Aspek", render: (item) => item.sub_aspect ? item.sub_aspect.name : "-" },
      { key: "aspect_name", header: "Aspek Induk", render: (item) => item.sub_aspect?.aspect ? `${item.sub_aspect.aspect.name} (${item.sub_aspect.aspect_code})` : (item.sub_aspect?.aspect_code || "-") },
      { key: "created_at", header: "Waktu Ditambahkan", render: (item) => item.created_at ? new Date(item.created_at).toLocaleString("id-ID") : "-" },
    ],
  },
